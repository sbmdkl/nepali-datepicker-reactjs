const bs = [];
bs[2000] = [ 2000, 30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31 ]; //365

bs[2001] = [ 2001, 31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30 ]; //365

bs[2002] = [ 2002, 31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30 ]; // 365

bs[2003] = [ 2003, 31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31 ]; //366
...
const fullDaysInEnglish = [ 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday' ];
...
const getNepaliNumber = (engNum) => {
	return engNum.toString().split('').map((n) => nepaliNumber[n]).join('');
};

const getEnglishNumber = (nepNum) => {
	return parseInt(nepNum.toString().split('').map((n) => nepaliNumber.indexOf(n)).join(''));
};
