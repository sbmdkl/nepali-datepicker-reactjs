...
const Header = (props) => {
	const renderYear = () =>
		Array(2100 - 2000).fill().map((_, idx) => 2000 + idx).map((val, i) => (
	...
	return (
		<div className={`${styles['header']} ${styles['theme-header-' + props.theme]}`}>
			...
			<form className={styles['header__mycontainer']}>
				...
				<select className={styles['header__mycontainer__select']} value={props.currentYear} onChange={props.bsYearJump}>
					{renderYear()}
				</select>
			</form>
			<p
